...
# Wait-times by satisfaciton (which is largely a sanity check!)
...
waittime_by_satisfaction_permit1_cor <- cor.test(as.integer(clean_data_tmp$first_permit_wait), clean_data_tmp$first_permit_timing_satisfaction, method = 'spearman', exact = FALSE)
waittime_by_satisfaction_permit2_cor <- cor.test(as.integer(clean_data_filt_for_second_tmp$second_permit_wait), clean_data_filt_for_second_tmp$second_permit_timing_satisfaction, method = 'spearman', exact = FALSE)
...
clean_data_above_1_year_and_clear_answer$new_position_prevent[grep('^No', clean_data_above_1_year_and_clear_answer$new_position_prevent)] <- 'No'
...
length(which(clean_data_above_1_year$new_position_prevent == 'No (but concerned)'))


# Statistical test on first vs. second wait-times.
wilcox.test(clean_data$first_permit_timing_satisfaction,
